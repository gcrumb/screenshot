...
var self;
var sslOpts = '--ignore-ssl-errors=yes';
...
RasterizerService.prototype.startService = function() {
		var rasterizer = this.rasterizer ? this.rasterizer : spawn(this.config.command, ['scripts/rasterizer.js', this.config.path, this.config.port, this.config.viewport]);
		rasterizer.stderr.on('data', function (data) {
				console.log('SlimerJS error: ' + data);
		});
		rasterizer.stdout.on('data', function (data) {
				console.log('SlimerJS output: ' + data);
		});
		...
		console.log('SlimerJS internal server listening on port ' + this.config.port);
		return this;
}
...
RasterizerService.prototype.killService = function() {
  if (this.rasterizer) {
    ...
    console.log('Stopping SlimerJS internal server');
  }
}
...
RasterizerService.prototype.checkHealth = function() {
  if (Date.now() - this.lastHealthCheckDate > this.sleepTime) {
    console.log('SlimerJS process is sleeping. Restarting.');
    this.restartService();
  }
}
